...
if (!token) {
  ...
  headers: {
    Authorization: token
      ...
      const productsRes = await axios.post( "http://localhost:3001/get-cart-items",{ productIds });
                ...
                <tbody>
                  {
                    products.map((product) => (
                      ...
                      <tr key={product._id}>
                        <td><img src={`http://localhost:3001/uploads/${product.image}`} alt={product.productName} className="product-image" /><br/>{product.productName}</td>
