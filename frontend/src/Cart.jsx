...
if (!token) {
  ...
  headers: {
    Authorization: token
      ...
      const productsRes = await axios.post( `${process.env.API_URL}/get-cart-items`,{ productIds });
                ...
                <tbody>
                  {
                    products.map((product) => (
                      ...
                      <tr key={product._id}>
                        <td><img src={`${process.env.API_URL}/uploads/${product.image}`} alt={product.productName} className="product-image" /><br/>{product.productName}</td>
