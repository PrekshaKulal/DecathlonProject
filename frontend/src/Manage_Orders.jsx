...
const [products, setProducts] = useState([]);
 ...
 useEffect(() => {
  ...
  return () => clearInterval(interval);
    ...
    try {
      const res = await axios.get(`${import.meta.env.API_URL}/my-orders`, {
...
const fetchProducts = async () => {
   ...
   try {
      const res = await axios.get(`${import.meta.env.API_URL}/products/ordered`, {
        headers: { Authorization: `Bearer ${token}` }
      });
...
const cancelOrder = async (id) => {
  try {
    await axios.put( `${import.meta.env.API_URL}/orders/cancel/${id}`,{},
      {
        headers: { Authorization: `Bearer ${token}` }
      }
    );
    ...
    <div key={ord._id} className="order-card">
      ...
      {ord.products.map((p, i) => (
        <div key={i} className="product-row">
          <img
            src={`${import.meta.env.API_URL}/uploads/${p.productId?.image}`}
